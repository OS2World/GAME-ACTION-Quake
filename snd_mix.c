...
extern char *dart_buffer0;
extern char *dart_buffer1;

void dart_write_buf0(void);

extern int last_written_buffer;

extern char *dart_buffers[];

volatile int last_painted_buffer = 0;
...
void S_TransferPaintBuffer(int endtime)
{
	...
	{
		unsigned char *out;
		out = (unsigned char *) dart_buffers[last_painted_buffer];
		while (count--)
		{
			val = (*p * snd_vol) >> 8;
			p+= step;
		...
		if (last_painted_buffer == 15)
			last_painted_buffer = 0;
		else
			last_painted_buffer++;
//		dart_write_buf0();
	}
