...
//
	...
	if (!dat)
		Sys_Error ("M_CachePic: failed to load %s", path);
...
void M_DrawTextBox (int x, int y, int width, int lines)
{
	...
	cx = x;
	cy = y;
	p = M_CachePic ("gfx/box_tl.lmp");
	M_DrawTransPic (cx, cy, p);
	p = M_CachePic ("gfx/box_ml.lmp");
	for (n = 0; n < lines; n++)
	{
		cy += 8;
		M_DrawTransPic (cx, cy, p);
	}
	p = M_CachePic ("gfx/box_bl.lmp");
	...
	{
		cy = y;
		p = M_CachePic ("gfx/box_tm.lmp");
		M_DrawTransPic (cx, cy, p);
		p = M_CachePic ("gfx/box_mm.lmp");
		for (n = 0; n < lines; n++)
		{
			cy += 8;
			if (n == 1)
				p = M_CachePic ("gfx/box_mm2.lmp");
			M_DrawTransPic (cx, cy, p);
		}
		p = M_CachePic ("gfx/box_bm.lmp");
		M_DrawTransPic (cx, cy+8, p);
		width -= 2;
		cx += 16;
	}
	...
	// draw right side
	cy = y;
	p = M_CachePic ("gfx/box_tr.lmp");
	M_DrawTransPic (cx, cy, p);
	p = M_CachePic ("gfx/box_mr.lmp");
	for (n = 0; n < lines; n++)
	{
		cy += 8;
		M_DrawTransPic (cx, cy, p);
	}
	p = M_CachePic ("gfx/box_br.lmp");
	M_DrawTransPic (cx, cy+8, p);
}
...
void M_Main_Draw (void)
{
	...
	M_DrawTransPic (16, 4, M_CachePic ("gfx/qplaque.lmp") );
	p = M_CachePic ("gfx/ttl_main.lmp");
	M_DrawPic ( (320-p->width)/2, 4, p);
	M_DrawTransPic (72, 32, M_CachePic ("gfx/mainmenu.lmp") );

	f = (int)(host_time * 10)%6;
	
	M_DrawTransPic (54, 32 + m_main_cursor * 20,M_CachePic( va("gfx/menudot%i.lmp", f+1 ) ) );
}
...
void M_SinglePlayer_Draw (void)
{
	...
	M_DrawTransPic (16, 4, M_CachePic ("gfx/qplaque.lmp") );
	p = M_CachePic ("gfx/ttl_sgl.lmp");
	M_DrawPic ( (320-p->width)/2, 4, p);
	M_DrawTransPic (72, 32, M_CachePic ("gfx/sp_menu.lmp") );

	f = (int)(host_time * 10)%6;
	
	M_DrawTransPic (54, 32 + m_singleplayer_cursor * 20,M_CachePic( va("gfx/menudot%i.lmp", f+1 ) ) );
}
...
void M_MultiPlayer_Draw (void)
{
	...
	M_DrawTransPic (16, 4, M_CachePic ("gfx/qplaque.lmp") );
	p = M_CachePic ("gfx/p_multi.lmp");
	M_DrawPic ( (320-p->width)/2, 4, p);
	M_DrawTransPic (72, 32, M_CachePic ("gfx/mp_menu.lmp") );

	f = (int)(host_time * 10)%6;
	
	M_DrawTransPic (54, 32 + m_multiplayer_cursor * 20,M_CachePic( va("gfx/menudot%i.lmp", f+1 ) ) );
}
...
void M_Setup_Draw (void)
{
	...
	p = M_CachePic ("gfx/bigbox.lmp");
	M_DrawTransPic (160, 64, p);
	p = M_CachePic ("gfx/menuplyr.lmp");
...
/* .........1.........2.... */
  ...
  "  This is a man-option  ",
  "for experts only, so get",
  " your sorry ass back up ",
  "  to the baby options!  "
};
...
void M_Net_Draw (void)
{
	...
	f = 32;
	if (serialAvailable)
		p = M_CachePic ("gfx/netmen1.lmp");
	else
		p = M_CachePic ("gfx/dim_modm.lmp");
	M_DrawTransPic (72, f, p);

	f += 19;
	if (serialAvailable)
		p = M_CachePic ("gfx/netmen2.lmp");
	else
		p = M_CachePic ("gfx/dim_drct.lmp");
	M_DrawTransPic (72, f, p);

	f += 19;
	if (ipxAvailable)
		p = M_CachePic ("gfx/netmen3.lmp");
	else
		p = M_CachePic ("gfx/dim_ipx.lmp");
	M_DrawTransPic (72, f, p);

	f += 19;
	if (tcpipAvailable)
		p = M_CachePic ("gfx/netmen4.lmp");
	else
		p = M_CachePic ("gfx/dim_tcp.lmp");
	M_DrawTransPic (72, f, p);

	if (m_net_items == 5)	// JDC, could just be removed
	{
		f += 19;
		p = M_CachePic ("gfx/netmen5.lmp");
		M_DrawTransPic (72, f, p);
	}
	...
	f = (int)(host_time * 10)%6;
	M_DrawTransPic (54, 32 + m_net_cursor * 20,M_CachePic( va("gfx/menudot%i.lmp", f+1 ) ) );
}
...
void M_Help_Draw (void)
{
	M_DrawPic (0, 0, M_CachePic ( va("gfx/help%i.lmp", help_page)) );
}
...
void M_NetStart_Change (int dir)
{
	...
	case 3:
		Cvar_SetValue ("teamplay", teamplay.value ? 0 : 1);
		break;
...
void M_Search_Draw (void)
{
	...
	p = M_CachePic ("gfx/p_multi.lmp");
	M_DrawPic ( (320-p->width)/2, 4, p);
	M_Print ((320/2) - ((12*8)/2), 40, "Searching...");
	if(slistInProgress)
	{
		NET_Poll();
		return;
